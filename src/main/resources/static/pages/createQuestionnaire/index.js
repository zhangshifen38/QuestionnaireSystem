...
onload = () => {
  ...
  projectId=$util.getPageParam('createQuestionnaire')
  console.log(projectId,'projectId')
  fetchProject(projectId)
}

const onCreateTemplate = () => {
  let text = document.getElementById("selectType").value
  console.log(text)
  if(text === "0"){
    alert("请选择调查类型！")
    return
  }
  $util.setPageParam('projectId',projectId)
  $util.setPageParam('type',text)
...
const fetchProject = (pid) => {
  ...
  $.ajax({
    ...
    success(res) {
      project = res.data
      console.log(project)
      $option = $('<option>').text(project.projectName).val(0).prop('selected',true)
